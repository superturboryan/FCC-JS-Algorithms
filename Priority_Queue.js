...
function PriorityQueue () {
    ...
    this.enqueue = function(elem) {
      if (collection[""+elem[1]] == null) {
        collection[""+elem[1]] = [elem[0]]
      }
      else {
        collection[""+elem[1]].push(elem[0])
      }
    }

    this.dequeue = function () {
      
      let keys = Object.keys(collection).sort((a,b)=>a-b)
      for (let key of keys) {
          if (collection[key].length != 0) {
              return collection[key].shift()
          }
      }
